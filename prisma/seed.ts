/**
 * Seed development MARLIN — deterministik & idempotent (upsert by unique key;
 * RAB tree hanya dibuat bila revisi belum ada). HANYA untuk dev/test.
 *
 * Data RAB riil: seed-data/*.json (hasil parse HPS via scripts/parse_hps.py).
 * Laporan harian demo dibuat relatif terhadap hari ini agar Command Center hidup.
 */
...
import { flattenParsedRab, grandTotal, type FlatNode } from "../src/lib/rab/flatten";
import type { ParsedRab } from "../src/lib/rab/parsed";
import { scheduleItems } from "../src/lib/scurve/generate";
import { ADMIN_MILESTONE_TEMPLATE } from "../src/lib/milestones/template";
import { withPpn, valueDone as calcValueDone } from "../src/lib/money";
...
if ((process.env.APP_ENV ?? "development") === "production") {
  console.error("TOLAK: seed dilarang di production.");
  process.exit(1);
}
...
async function main() {
  ...
  for (const p of PACKAGES) {
    for (const slug of p.slugs) {
      files.set(slug, JSON.parse(readFileSync(join(__dirname, "..", "seed-data", `${slug}.json`), "utf8")) as SeedFile);
    }
  }
