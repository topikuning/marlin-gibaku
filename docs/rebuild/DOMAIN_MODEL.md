...
- Detail administrasi dalam tiap stage = **AdminMilestone** (checklist KKP 45 item — `src/lib/milestones/template.ts`) yang statusnya bisa auto-suggest dari upload dokumen tapi butuh verifikasi untuk milestone kritis. Upload dokumen ≠ otomatis milestone selesai.
...
### Pengendalian terpadu — temuan, inspeksi, verifikasi eksternal (DECISIONS 426)
- `Finding` (temuan PEMERIKSA — bukan pengganti `Issue`): locationId, source
  (`inspeksi`|`laporan_harian`|`dokumen`|`manual`), inspectionId?/reportId?,
  lineageKey? (+ workItemName snapshot), category, severity, findingDate,
  dueDate?, status (`baru`→`menunggu_klarifikasi`→`ditindaklanjuti`→
  `menunggu_verifikasi`→`selesai`→`dibuka_kembali`), assignedToId?/assignedName,
  raisedBy, closedBy/At, reopenCount. + `FindingStatusHistory` (append-only,
  trigger DB), `FindingClarification` (Q/A), `FindingNote` (tindak lanjut).
- `Inspection` — inspeksi lapangan Wakil PPK: locationId, inspectorId, tanggal,
  catatan/rekomendasi, `draft`→`final` (draft hanya bisa diubah pemeriksanya).
- `EvidenceLink` — bukti = TAUTAN ke `Photo` ATAU `Document` existing (XOR +
  wajib induk finding/inspection/clarification via CHECK constraint DB; tanpa
  duplikasi berkas), verifStatus per-tautan (`belum`|`diterima`|`ditolak`).
- `ReportVerification` — verifikasi EKSTERNAL laporan harian, append-only
  (trigger DB); baris terakhir per laporan = keadaan; TIDAK menyentuh
  `DailyReport.status` maupun angka resmi.
- Kesiapan termin/PHO/FHO dan EWS = DERIVED murni (`lib/kesiapan/`, `lib/ews/`),
  tidak ada tabel.
...
- `Alert` — MATI: tidak pernah ditulis/dibaca kode sejak rebuild, dan
  DECISIONS 426 memutuskan EWS dihitung on-the-fly (`lib/ews/`), bukan
  disimpan. Tabelnya belum di-drop; jangan dihidupkan lagi.
