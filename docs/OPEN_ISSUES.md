...
- 🟢 **Kronologi belum diuji di basis data sungguhan.** `susunKronologi` dan
  bahan AI-nya diuji unit; `ambilKronologi` (query Prisma) belum punya uji
...
## 🔴 RAB-ADD-01 · Template adendum terbitan MARLIN diimpor balik → "676 item baru · 676 item hilang"

Dilaporkan user 2026-09-12 dengan tangkapan layar. Template adendum diunduh dari
MARLIN (revisi aktif #1, 903 item, Rp 3.723.269.226), diisi kolom volumenya,
diimpor balik ke DRAFT adendum di lokasi yang sama. Pratinjaunya:
...
yang tidak saling kenal antara berkas dan RAB aktif.

**Yang SUDAH dibuktikan dari berkas user (template + ekspor RAB aktif, keduanya
terbitan MARLIN)** — semuanya BERSIH, jadi penyebabnya bukan di sini:

| diuji | hasil |
|---|---|
| kolom identitas (K) di template vs `lineageKey` hasil `parseAdendumTemplate` | 1097 sama, 0 beda |
| template vs ekspor RAB aktif (kode + nama) | 21 beda, seluruhnya kosmetik (spasi ganda, sufiks `#2`) |
| `bandingkanTerhadapAktif` dengan sisi "aktif" dirakit dari kolom kontrak template itu sendiri | **0 item baru**, 723 tetap, 180 volume berubah |
| `samakanLineage` pada identitas yang sudah benar | 10 kunci disentuh, 0 item baru |

**Yang belum diketahui**: nilai `lineageKey` yang BENAR-BENAR tersimpan di
`rab_nodes` untuk revisi #1 di produksi. Semua bukti di atas menunjuk ke sana —
676 dari 903 kunci di DB berbeda dari yang disalin template, padahal template
menyalinnya dari DB.

**Dugaan yang belum terbukti**: revisi #1 pernah ditulis ulang DI TEMPAT sesudah
template diunduh. Nomor revisi tetap #1 dan totalnya tetap sama persis, jadi
tidak ada satu pun tanda di layar — tapi kuncinya berubah. Perubahan parser
adendum 2026-09-07 (DECISIONS 542/543: induk berharga naik jadi item, anaknya
naik jadi saudara) menghasilkan kunci berbeda untuk berkas yang SAMA, jadi impor
ulang berkas yang sama pun cukup untuk memutus tautannya.

**Langkah berikutnya**: bandingkan `lineageKey` tersimpan vs kolom K template
untuk satu item yang dilaporkan "baru" (mis. `I · 6.1.a`). Kalau berbeda, dugaan
di atas terbukti dan perbaikannya di sisi penulisan revisi, bukan di importir.

Tidak ada data yang rusak: ini pratinjau, dan user tidak melanjutkan.
