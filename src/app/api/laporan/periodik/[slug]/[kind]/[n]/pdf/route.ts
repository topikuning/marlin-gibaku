...
import { renderPeriodikPdf } from "@/lib/pdf/periodik";
import type { PeriodKind } from "@/lib/periodic-report";

/** Unduh Laporan Mingguan/Bulanan (PDF ringkas server-side). Auth → akses lokasi. DECISIONS 126. */
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string; kind: string; n: string }> }) {
  ...
  const result = await renderPeriodikPdf(loc.id, kind as PeriodKind, nNum);
