    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {run.status === "siap" && LAPORAN_TEMPLATE_FOR_KIND[run.runKind] ? (
          <Link
            href={`/pengendalian/insight/reports?template=${LAPORAN_TEMPLATE_FOR_KIND[run.runKind]}&scopeIds=${(run.scopeIds as string[]).join(",")}`}
                  ...
                  {official.rows.map((r) => (
                    <tr key={r.locationId} className="border-b border-border-muted">
                      <td className="px-3 py-1.5">
                        <Link href={`/proyek/lokasi/${r.slug}/progress`} className="hover:underline">
                          {r.name}
                        </Link>
                      </td>
                    ...
                    <ul className="mt-1 space-y-1">
                      {l.reports.map((r) => (
                        <li key={r.id} className="rounded-md border border-border-muted px-2.5 py-1.5">
                          ...
                          <Link href={`/proyek/lokasi/${l.slug}/harian/${r.date}`} className="text-xs text-primary hover:underline">
                            Buka laporan →
                          </Link>
                        </li>
                      ))}
                    ...
                    <ul className="mt-1 space-y-1">
                      {l.activities.map((a) => (
                        <li key={a.id} className="rounded-md border border-border-muted px-2.5 py-1.5">
                          ...
                          <Link href={`/proyek/lokasi/${l.slug}/kegiatan`} className="text-xs text-primary hover:underline">
                            Buka kegiatan →
                          </Link>
                        </li>
                      ))}
