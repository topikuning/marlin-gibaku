      ...
      <CardHeader
        ...
        action={
          conversation ? (
            ...
            <Link
              href={`/pengendalian/insight/reports?template=wa_update&scopeIds=${conversation.scopeIds.join(",")}`}
                ...
                {m.role !== "user" && (m.citations.length > 0 || m.confidence != null) ? (
                  <p className="mt-1.5 border-t border-border-muted pt-1 text-[11px] text-ink-faint">
                    ...
                    {m.runId ? (
                      <>
                        {" · "}
                        <Link href={`/pengendalian/insight/run/${m.runId}`} className="text-primary hover:underline">
                          detail run
                        </Link>
                      </>
                    ) : null}
