    ...
    <div className="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)]">
      <Card className="self-start">
        ...
        <CardBody className="space-y-1">
          <Link href="/pengendalian/insight/ask" className="block rounded-md border border-dashed border-border px-2 py-1.5 text-sm text-primary hover:border-primary">
            + Percakapan baru
          </Link>
          {conversations.map((c) => (
            <Link
              key={c.id}
              href={`/pengendalian/insight/ask?c=${c.id}`}
