 ...
 * (DECISIONS 135): Perusahaan (profil + logo + kop), Kontak WA, Pengguna.
...
export default async function MasterLayout({ children }: { children: React.ReactNode }) {
  ...
  if (can(user.role, "exec_report.send")) tabs.push({ label: "Kontak WA", href: "/master/kontak-wa" });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Master Data"
        description="Data referensi lintas modul: profil perusahaan (logo & kop surat), kontak WhatsApp tujuan laporan, dan akun pengguna."
      />
      <LinkTabs items={tabs} />
      {children}
    </div>
