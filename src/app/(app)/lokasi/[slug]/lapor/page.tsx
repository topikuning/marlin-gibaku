...
export default async function LaporPage({
  ...
  return (
    <>
      <Link href={`/lokasi/${slug}`} className="mb-4 inline-block text-sm text-[#3A4E63] hover:underline">
        ← Detail Lokasi
      </Link>
      ...
      {myDrafts.length === 0 ? (
        <p className="text-sm text-[#8a9199]">Belum ada laporan.</p>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-[#EAE2D2]">
          <table className="w-full min-w-[560px] text-sm">
            ...
            <tbody>
              {myDrafts.map((d) => (
                <tr key={d.id} className="border-b border-[#F0EADD] last:border-0">
                  <td className="px-4 py-2.5">
                    <span className="font-mono text-[11px] text-[#8a9199]">{d.rabItem.code}</span>{" "}
                    <span className="text-[#1f2b38]">{d.rabItem.name}</span>
                  </td>
                  ...
                  <td className="px-4 py-2.5">
                    ...
                    {d.rejectedReason && (
                      <span className="ml-2 text-xs text-[#C1442E]">{d.rejectedReason}</span>
                    )}
                  </td>
                </tr>
              ))}
