  ...
  return (
    <div className="space-y-3">
      ...
      {ringkas ? (
        <div className="rounded-lg border border-success-border bg-success-soft px-4 py-3">
          <p className="flex items-center gap-2 text-sm font-medium text-ink">
            <BookText aria-hidden className="size-4 shrink-0 text-success" />
            {ringkas.name}
          </p>
      ...
      <Button type="button" variant="secondary" size="sm" loading={busy} onClick={jalankan}>
        ...
        {ringkas ? "Periksa & segarkan basis AHSP" : "Muat basis AHSP"}
      </Button>

      <p className="text-[12px] text-ink-muted">
        Sumber: Surat Edaran Dirjen Bina Konstruksi No. 47/SE/Dk/2026, Lampiran II–VI. Aman ditekan
        berulang — berkas yang isinya sama tidak ditulis ulang.
      </p>
    </div>
  );
}
