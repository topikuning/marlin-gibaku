...
export async function uploadDocumentAction(
  ...
  try {
    ...
    if (d.locationSlug) revalidatePath(`/proyek/lokasi/${d.locationSlug}/dokumen`);
    if (d.packageId) revalidatePath(`/proyek/paket/${d.packageId}/dokumen`);
    if (d.amendmentId && d.packageId) revalidatePath(`/proyek/paket/${d.packageId}/kontrak`);
    return {
...
function revalidateDocument(t: { documentId: string; locationSlug?: string; packageId?: string }): void {
  revalidatePath("/dokumen-laporan/dokumen");
  revalidatePath(`/dokumen-laporan/dokumen/${t.documentId}`);
  if (t.locationSlug) revalidatePath(`/proyek/lokasi/${t.locationSlug}/dokumen`);
  if (t.packageId) {
    revalidatePath(`/proyek/paket/${t.packageId}/dokumen`);
    revalidatePath(`/proyek/paket/${t.packageId}/administrasi`);
  }
}
