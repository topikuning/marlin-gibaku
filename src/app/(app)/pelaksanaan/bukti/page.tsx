...
export default async function FotoLapanganPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const qs = (patch: Record<string, string | number | undefined>) => {
    ...
    const s = p.toString();
    return s ? `/pelaksanaan/bukti?${s}` : "/pelaksanaan/bukti";
  };
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          can(user.role, "photo.archive_purge") ? (
            <Link href="/administrasi/sistem/arsip-foto" className="text-sm text-primary hover:underline">
              Arsip foto asli →
            </Link>
          ) : null
        }
      ...
      {/* Filter */}
      <form method="get" action="/pelaksanaan/bukti" className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-surface p-2.5 shadow-sm">
        <div className="w-52">
