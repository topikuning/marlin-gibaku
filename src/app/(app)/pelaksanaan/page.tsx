...
import {
  ...
  ISSUE_SEVERITY_TONE,
} from "@/app/(app)/proyek/lokasi/[slug]/issue-labels";
      ...
      {summaries.map((s) => (
        <Card key={s.id}>
          ...
          <CardBody className="space-y-4">
            ...
            <Link
              href={`/proyek/lokasi/${s.slug}/harian/${todayKey}`}
            ...
            {s.corrections.map((c) => (
              <Link
                key={c.dateKey}
                href={`/proyek/lokasi/${s.slug}/harian/${c.dateKey}`}
                ...
                {[...s.last7Days].reverse().map((d) => {
                  ...
                  return (
                    <li key={d.dateKey} className="min-w-0 flex-1">
                      <Link
                        href={`/proyek/lokasi/${s.slug}/harian/${d.dateKey}`}
