  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <PageHeader
        ...
        compactMobile
        breadcrumb={[
          { label: "Hari Ini", href: "/pelaksanaan" },
      ...
      {isFuture ? (
        <Banner
          ...
          description={
            <Link href={`/proyek/lokasi/${slug}/harian/${todayKey}`} className="font-medium text-primary hover:underline">
      ...
      {report && (status === "dikirim" || status === "disetujui") ? (
        <p className="text-right">
          <ButtonLink
            href={withBackTo(`/cetak/harian/${slug}/${date}`, `/proyek/lokasi/${slug}/harian/${date}`)}
            target="_blank"
            variant="secondary"
            size="sm"
          >
        ...
        <CardBody className="px-0 py-0">
          <ul className="divide-y divide-border">
            {data.recentDays.map((d) => (
              <li key={d.dateKey}>
                <Link
                  href={`/proyek/lokasi/${slug}/harian/${d.dateKey}`}
