...
export default async function ImporRekapHarianPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <ButtonLink href={`/proyek/lokasi/${slug}/harian`} variant="ghost" size="sm" className="self-start">
      ...
      <Card>
        ...
        <CardBody className="space-y-2 text-sm text-ink-muted">
          <ol className="list-decimal space-y-1 pl-5">
            <li>
              Unduh{" "}
              <Link href={`/proyek/lokasi/${slug}/harian/import/template`} className="font-medium text-primary hover:underline">
                template Excel
              </Link>{" "}
