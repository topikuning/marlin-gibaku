...
const baris = (
  ...
  wa: Date | null,
): BarisTersaring => ({ slug, drive: { status: drive }, waSentAt: wa });

const KIRIM = new Date("2026-08-06T09:00:00.000Z");

const ISI: BarisTersaring[] = [
  baris("pasir", "sukses", KIRIM), // sudah semua
  baris("kedungmutih", null, KIRIM), // drive belum, wa sudah
  baris("batah-timur", "gagal", null), // drive gagal, wa belum
  baris("tengket", null, null), // belum semua
];
...
describe("tautFilter", () => {
  ...
  it("saringan lain dipertahankan saat satu diubah", () => {
    const t = tautFilter("2026-08-06", { lokasi: "pasir", drive: "belum", wa: "semua" }, { wa: "sudah" });
